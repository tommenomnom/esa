...
void loop()
{
  ...
  if (millis() - start_time >= 7000 && millis() - start_time < 17000) {
  ...
  //EVERY_N_SECONDS( 10 ) { nextPattern(); } // change patterns periodically
    ...
    if (millis() - start_time >= 17000 && millis() - start_time < 32000) {
  // Call the current pattern function once, updating the 'leds' array
  gPatterns[gCurrentPatternNumber]();
  ...
  //EVERY_N_SECONDS( 10 ) { nextPattern(); } // change patterns periodically
      ...
      if (millis() - start_time >= 32000 && millis() - start_time < 47500) {
  // Call the current pattern function once, updating the 'leds' array
  gPatterns[gCurrentPatternNumber]();
  ...
  //EVERY_N_SECONDS( 10 ) { nextPattern(); } // change patterns periodically
    ...
    else if (millis() - start_time >= 48000) {
    fill_solid(leds, NUM_LEDS, CRGB::Black);
    //start_time = millis();   
    FastLED.show();
  }
  ...
  if (millis() - start_time >= 5500 && millis() - start_time < 6500) {
   ...
   if (millis() - start_time >= 7000 && millis() - start_time < 48000) {
  ...
  if (millis() - start_time >= 48500 && millis() - start_time < 60000) {
    //Serial.write("stop");
    analogWrite(vib, 0);
  }
