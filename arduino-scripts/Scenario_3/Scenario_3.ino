...
void loop()
{
  ...
  if (millis() - start_time < 8000){
    ...
    FastLED.show();
      ...
      if (millis() - start_time >= 7500 && millis() - start_time < 12500) {
  ...
  //EVERY_N_SECONDS( 10 ) { nextPattern(); } // change patterns periodically
    ...
    if (millis() - start_time >= 12500 && millis() - start_time < 27500) {
  // Call the current pattern function once, updating the 'leds' array
  gPatterns[gCurrentPatternNumber]();
  ...
  //EVERY_N_SECONDS( 10 ) { nextPattern(); } // change patterns periodically
      ...
      if (millis() - start_time >= 27500 && millis() - start_time < 55000) {
  // Call the current pattern function once, updating the 'leds' array
  gPatterns[gCurrentPatternNumber]();
  ...
  if (millis() - start_time >= 4000 && millis() - start_time < 5000) {
   ...
   if (millis() - start_time >= 7500 && millis() - start_time < 55500) {
  ...
  if (millis() - start_time >= 55500 && millis() - start_time < 60000) {
    //Serial.write("stop");
    analogWrite(vib, 0);
  }
...
void PulsateVibration(int vib_int_min, int vib_int_max, bool light) {
  ...
  //EVERY_N_SECONDS( 10 ) { nextPattern(); } // change patterns periodically
        ...
        else if (millis() - start_time >= 55000) {
    fill_solid(leds, NUM_LEDS, CRGB::Black);
    //start_time = millis();   
    FastLED.show();
  }
...
void sinelon_oneway()
{
  ...
  if (millis() - start_time >= 32500 && millis() - start_time < 55000) {
